...
#define OFPMT_STANDARD_LENGTH   96
...
//*********************************
// Added by Bence Ladoczki
    OFPAT_SET_MPLS_LABEL = 28,  /* MPLS label */
    OFPAT_SET_DL_DST =29,       /* Ethernet destination address. */
    OFPAT_SET_NW_DST =30,       /* IP destination address. */
    OFPAT_SET_PARITY_MPLS_LABEL = 31,  /* MPLS label parity */
    OFPAT_SET_MPLS_LABEL_FROM_COUNTER = 32,  /* MPLS label from counter*/
    OFPAT_XOR_ENCODE = 33,  /* XOR encoding*/
    OFPAT_XOR_DECODE = 34,  /* XOR decoding*/
...
//Added by Bence Ladoczki
struct ofp_action_parity_mpls_label {
        uint16_t type;    /* OFPAT_SET_PARITY_MPLS_LABEL. */
        uint16_t len;     /* Length is 8. */
        uint8_t pad[4];

};
OFP_ASSERT(sizeof(struct ofp_action_parity_mpls_label) == 8);
...
struct ofp_action_set_mpls_label_from_counter {
        uint16_t type;    /* OFPAT_SET_PARITY_MPLS_LABEL. */
        uint16_t len;     /* Length is 8. */
        uint8_t pad[4];

};
OFP_ASSERT(sizeof(struct ofp_action_set_mpls_label_from_counter) == 8);
struct ofp_action_xor_encode {
        uint16_t type;    /* OFPAT_SET_PARITY_MPLS_LABEL. */
        uint16_t len;     /* Length is 8. */
	uint32_t label_a; /* Label_A */
	uint32_t label_b; /* Label_B */
        uint8_t pad[4];

};
OFP_ASSERT(sizeof(struct ofp_action_xor_encode) == 16);
struct ofp_action_xor_decode {
        uint16_t type;    /* OFPAT_SET_PARITY_MPLS_LABEL. */
        uint16_t len;     /* Length is 8. */
	uint32_t label_a; /* Label_A */
	uint32_t label_b; /* Label_B */
...
OFP_ASSERT(sizeof(struct ofp_action_xor_decode) == 16);
